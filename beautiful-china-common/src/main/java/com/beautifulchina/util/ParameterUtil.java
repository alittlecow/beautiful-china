...
import com.beautifulchina.constants.BaseErrorCode;
...
public class ParameterUtil {
    ...
    public static void validateParamsValues(Map<String, String> map, final String... params) throws RedisException {
        for (String param : params) {
            ...
            if (null == val || "".equals(val) || "undefined".equals(val)) {
                throw new RedisException(BaseErrorCode.PARAM_EMPTY_ERROR);
            }
        }
    }
     ...
     * @return
     */
        public static boolean isCanadaTel(String mobileno) {
        mobileno = mobileno.trim();
        String regex = "/^((\\d{3}-)?(\\d{7})|^(\\d{7}))$/";
        return mobileno.matches(regex);
    }
